...
| ハードリミットマスタ（スライス注文設定） | `/slice-settings`（照会・変更） | `src/api/hardLimits.js` | 楽観ロックの 409 が `openapi.json` に未宣言（PUT の description にだけ記載。実 API では実装済み）。`備考` と `スライス有効フラグ` は**省略するとサーバ既定に落ちる**（備考は NULL、有効フラグは 1）ので、現在値を送り返して保持している |
...
- `/slice-settings` … 日本語キー / 拒否は 422 の `HTTPValidationError`（不合格の項目を全部まとめて返す）と
  409 の `ErrorResponse`
