...
ここだけが**バックエンドの形**（`/slice-settings` というパス・日本語キー・`スライス有効フラグ` の
...
| ID | 前提 | 操作 | 期待結果 | 状態 |
|---|---|---|---|---|
| HLA-01 | 既定モック | `fetchHardLimits()` を呼ぶ | `GET /api/slice-settings` を呼ぶ。クエリは付けない（単一リソースなので絞り込みが無い） | 実装済 |
...
| HLA-06 | 既定モック | `updateHardLimits()` を呼ぶ | `PUT /api/slice-settings` の本文が日本語キー 6 項目（`市場関与率` / `大口数量閾値` / `大口金額閾値` / `スライス有効フラグ` / `備考` / `更新日時`）になる | 実装済 |
