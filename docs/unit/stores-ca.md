...
この一覧は**取得・登録・更新**を持ち、削除はまだ持たない。`useCrudList` は渡さなかった操作の
...
| CAS-09 | 既定モック | ストアの公開名を読む | `create` / `update` とその状態（`creating` / `updating` / `validationErrors` / `updateValidationErrors`）を公開し、`remove` / `deleting` は持たない（まだ無い操作を、できるように見せない） | 実装済 |
