...
| MHS-08 | 既定モック（56 件）、`load()` 済み | 一覧に無い日付で `create({ date, reason })` を呼ぶ | 登録した 1 件が返り、`createError` は null。一覧が読み直されて `total` が 1 増え、その日付が `items` に入る | 実装済 |
| MHS-09 | POST が 409（`message` 付き）を返す | `create()` を呼ぶ | 戻り値が null、`createError` に status 409 と message が入る。`items` / `total` は変わらない | 実装済 |
