...
import { stocks } from '../src/mocks/fixtures/stocks'
import { formatQuantity, formatUsdUnit } from '../src/utils/format'
import { mockApi } from './helpers/mockApi'

// シナリオ: docs/e2e/stocks.md（タイトル先頭の [SM-xx] が対応 ID）
...
// src/stores/stocks.js の STOCKS_PAGE_SIZE と同じ値（実 API も 1 ページ 50 件固定）。
...
const toRow = (stock) => ({
  ...
  nameEn: stock.銘柄名_英字 ?? '',
  name: stock.銘柄名 ?? '',
  previousClose: stock.前日終値 ?? null,
  previousVolume: stock.前日出来高 ?? null,
  averageVolume: stock.平均出来高 ?? null,
...
const ORDER_ROUTE = '0' // src/utils/stockTypes.js の ORDER_ROUTE_OPTIONS（0: みずほ証券）
...
// DataTable の列順（StockListView.vue の columns）。相場の 3 列の位置
...
function rowsOf(page) {
  return page.getByTestId('stocks-table').getByTestId('data-table-row')
}
...
test.describe('銘柄マスタ一覧', () => {
  test('[SM-01] サイドメニューから開くと一覧と件数が表示される', async ({ page }) => {
    ...
    await expect(page.getByTestId('stocks-reload')).toBeVisible()

    await expect(page.getByTestId('stocks-count')).toHaveText(`${TOTAL} 件`)
  ...
  test('[SM-02] 「次のページ」を押すと 2 ページ目が表示される', async ({ page }) => {
    ...
    const pagination = page.getByTestId('stocks-pagination')
  ...
  test('[SM-03] ティッカーコードで絞り込むと URL と一覧に反映される', async ({ page }) => {
    ...
    await page.getByTestId('stocks-stock-code').fill(TICKER)
    await page.getByTestId('stocks-search-submit').click()

    await expect(page).toHaveURL(new RegExp(`stock_code=${TICKER}`))
    await expect(page.getByTestId('stocks-count')).toHaveText(`${byTicker.length} 件`)

    const rows = rowsOf(page)
    await expect(rows).toHaveCount(byTicker.length)
    await expect(rows.first()).toContainText(byTicker[0].stockCode)
  })
  ...
  test('[SM-04] 取引可否で絞り込むと取引不可の行だけになる', async ({ page }) => {
    ...
    await page.getByTestId('stocks-regulation').selectOption(CLOSED.regulation)
    await page.getByTestId('stocks-search-submit').click()

    await expect(page).toHaveURL(new RegExp(`regulation=${CLOSED.regulation}`))
    await expect(page.getByTestId('stocks-count')).toHaveText(`${byRegulation.length} 件`)
  ...
  test('[SM-05] 預託先区分と VWAP対象区分は組み合わせて効く', async ({ page }) => {
    ...
    await page.getByTestId('stocks-order-route').selectOption(ORDER_ROUTE)
    await page.getByTestId('stocks-vwap-target').selectOption(VWAP_TARGET)
    await page.getByTestId('stocks-search-submit').click()
    ...
    await expect(page.getByTestId('stocks-count')).toHaveText(`${byRouteAndVwap.length} 件`)
  ...
  test('[SM-06] 「クリア」を押すと絞り込みが解除される', async ({ page }) => {
    ...
    await page.getByTestId('stocks-search-clear').click()

    await expect(page).toHaveURL(new RegExp(`${PATH}$`))
    await expect(page.getByTestId('stocks-count')).toHaveText(`${TOTAL} 件`)
    await expect(rowsOf(page)).toHaveCount(PAGE_SIZE)
    await expect(page.getByTestId('stocks-stock-code')).toHaveValue('')
  })
  ...
  test('[SM-07] 銘柄名では絞れず空状態になる', async ({ page }) => {
    ...
    await page.getByTestId('stocks-stock-code').fill(firstRow.name)
    await page.getByTestId('stocks-search-submit').click()

    await expect(page.getByTestId('stocks-empty')).toHaveText('該当する銘柄はありません。')
    await expect(page.getByTestId('stocks-table')).toBeHidden()
  ...
  test('[SM-08] API がエラーを返したときエラー表示と再試行ボタンが出る', async ({ page }) => {
    await mockApi(page, [{ path: '*/api/stocks', status: 500, body: { detail: ERROR_MESSAGE } }])
    await page.goto(PATH)

    const error = page.getByTestId('stocks-error')
    ...
    await expect(page.getByTestId('stocks-description')).toBeVisible()
    await expect(page.getByTestId('stocks-search')).toBeVisible()
  })
  ...
  test('[SM-10] 列順が仕様どおりで行の操作が無い', async ({ page }) => {
    ...
    await expect(page.getByTestId('stocks-table').locator('th')).toHaveText([
      '銘柄コード',
      'ティッカーコード',
      '銘柄名（英語）',
      '銘柄名（日本語）',
    ...
    await expect(page.getByTestId('stocks-add')).toHaveCount(0)
  ...
  test('[SM-12] ブラウザバックで前のページに戻る', async ({ page }) => {
    ...
    await page.getByTestId('stocks-pagination').getByRole('button', { name: '次のページ' }).click()
    await expect(page).toHaveURL(new RegExp(`offset=${PAGE_SIZE}`))
