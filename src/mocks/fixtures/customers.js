 ...
 * docs/api/openapi.json の AccountItem（m_口座情報 の行モデル）に合わせてある
 * （プロパティ名は日本語、口座番号は integer、取消区分・ユーザー操作フラグは 0/1）。
 * ブラウザ(MSW worker)・単体テスト・E2E で共用する。
 *
 * `GET /customers` の応答は openapi 上 `customers: array<object>` で中身が未定義なので、
 * 同じ m_口座情報 由来の AccountItem の形を前提に置いている。
 *
